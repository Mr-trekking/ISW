...
)
{
    ...
    if (fieldHeader.typeHeaderOk<fieldType>(true))
    {
        ...
        // the location of the centre of the pycnocline
        const scalar z_pyc = 0.4;

        // the pycnocline thickness
        const scalar d_pyc = 0.007;
        ...
        forAll(field,cellI)
        {
            ...
            scalar index = (mesh.C()[cellI].z() - eta - h2) / d_pyc;           
            scalar ratio = (Foam::pow(2.718,index) - Foam::pow(2.718,-index)) / (Foam::pow(2.718,index) + Foam::pow(2.718,-index));

            if (mesh.C()[cellI].z() > z_pyc)
            {
                field[cellI] = value * (0.5 * (rho1 + rho2) - 0.5 * (rho2 - rho1) * ratio);
            }
	    else
            {
                field[cellI] = value * (0.5 * (rho1 + rho2) - 0.5 * (rho2 - rho1) * ratio);
...
int main(int argc, char *argv[])
{
    ...
    const word dictName("setRhoFieldsDict");
